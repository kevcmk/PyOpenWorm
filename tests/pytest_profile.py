...
class Benchmark():
    ...
    def __init__(self, int_ops=7000000, float_ops=7500000, atan_ops=75000, sort_ops=5000):
        # These values run approx. 1 second on Macbook Air 2014 (1.4GHz i5)
        # They ought to be synchronized to the Travis-CI Machine
        self.times = []
        self.int_ops = int_ops
        self.float_ops = float_ops
        self.atan_ops = atan_ops
        self.sort_ops = sort_ops

    def update(self):
        int_time = timeit.timeit('100 * 99 + 5', number=self.int_ops)
        float_time = timeit.timeit('100.5 * 99.2 * 98.6 * 97.1', number=self.float_ops)
        atan_time = timeit.timeit('import random, math; math.atan2(random.random(), random.random())', number=self.atan_ops)
        sort_time = timeit.timeit('sorted(reversed(xrange(1000)))', number=self.sort_ops)
        threshold = int_time + float_time + atan_time + sort_time
        self.times.append(threshold)
    @property
    def average(self):
        if len(self.times) is 0:
            raise ValueError('Cannot request benchmark before calling update.')
...
def pytest_addoption(parser):
    ...
    profile_group.addoption('--travis-profile', dest='profile', action='store_true',
                            default=False, help='Generate function profiles.')

def pytest_configure(config):
    """
    Called before tests are collected.
    """
    global enabled, benchmark
...
def pytest_unconfigure(config):
    ...
    benchmark.update()
    threshold = benchmark.average

    print '[Machine Benchmark Time]: {:.2f}s'.format(threshold)

    violating_functions = []

    for func in function_profile_list:
        if func.cumulative_time > threshold:
            print '   [Performance Warning]: {} @ {:.2f}s'.format(func.function_name, func.cumulative_time)
            violating_functions.append(func)
